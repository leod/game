...
_backtrace(struct output_buffer *ob, struct bfd_set *set, int depth , LPCONTEXT context)
{
	...
	while(StackWalk(IMAGE_FILE_MACHINE_I386, 
        ...
        // Changed to use GetModuleBaseNameA by Leo
		if (module_base && 
			GetModuleBaseNameA(process, (HMODULE)module_base, module_name_raw, MAX_PATH)) {
        ...
        // Added by Leo
        {
            char * fileStart = strstr(file, "src/");
...
exception_filter(LPEXCEPTION_POINTERS info)
{
    ...
    if (file) {
        char buffer[1000];
        time_t t=time(NULL);
        strftime(buffer,1000,"Crash at: %A, %B %d, %Y at %H:%M:%S.\n",localtime(&t));
        fputs(buffer, file);
        fputs(g_output, file);
        fputs("\n", file);
        fclose(file);
